...
var data = {
  ...
  // `tracks` holds the six tracks of the drum machine.  Each
...
setInterval(function() {
  ...
  // Find all the tracks where the curent step is on.  Play their
  // sounds.
  data.tracks
...
// Get the `screen` object.  This is a bundle of functions that, when
// called, draw in the canvas element created in index.html.
var screen = document.getElementById("screen").getContext("2d");

// **draw()** draws the drum machine.
// Called once at the beginning of the program.  Is then called 60
// times a second forever because of the `requestAnimationFrame()`
// call below.
(function draw() {
...
// clicks turn track buttons on and off.
...
function note(audio, frequency) {
  return function() {
    ...
    // Web audio works by connecting nodes togther in chains.  The output of one
    // node becomes the input to the next.  Thus, sound can be created and modified.
    chain([
      ...
      // over the duration of the tone.  This produces an echoey effect.
...
function kick(audio) {
  return function() {
    ...
    // Set the initial frequency of the drum at a low `160`.  Reduce it to 0 over the duration of the sound.
    // This produces that BBBBBBBoooooo..... drop effect.
...
// **createSineWave()** returns a sound node that
// plays a sine wave of `duration` seconds.
function createSineWave(audio, duration) {

  // Create an oscillator (a sound wave that goes up and down).
  var oscillator = audio.createOscillator();

  // Make the oscillator a sine wave.  Different types of wave
  // produces different characters of sound.  A sine wave produces
  // a pure tone.
  ...
  // Tell the sine wave to stop playing after `duration` seconds
  // have passed.
...
// **rampDown()** takes a `value`, sets it to `startValue` and
// reduces it to almost `0` in `duration` seconds.  `value` might be the volume
...
// **createAmplifier()** returns a sound node that controls the volume of the sound
// coming into it.  The volume is started at `startValue` and ramped down
// in `duration` seconds to almost `0`.
...
// **chain()** connects an array of `soundNodes` into a chain.
// If there are three nodes in `soundNodes`, the first will be joined to the second
...
// **createTrack()** returns an object that represents a track.
// This contains an array of 16 steps.  Each of these are either on (`true`) or off (`false`).
// It contains `color`, the color to draw buttons when they are on.  It contains `playSound`, the function
...
// **isPointInButton()** returns true if `p`, the coordinates of a mouse click
// are inside the button at `column` and `row`.
